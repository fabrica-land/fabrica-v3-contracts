...
// src/FabricaAttributeOracle.sol as of fabrica-v3-contracts commit 062f049, which is the
// exact source of the round-1 fact store deployed on Sepolia at
// 0xFfA7535eF090C9193f44399843a05b60808ffC0D. Verified by compiling this file and
// diffing its runtime bytecode against `cast code` on that address: 13,458 bytes each,
// with 147 differing bytes, every one of them inside a slot listed in the artifact's
// immutableReferences (the knob immutables, e.g. historyDepth = 48 = 0x30).
//
// Present only so the model can show a measured deployed-versus-main delta under one
// harness. It is not built into anything and nothing in src/ references it.
// Renamed to avoid colliding with the live contract of the same name.
...
contract DeployedRound1FactStore is Ownable2Step, EIP712 {
