...
/// `_everMinted` flag (set before the callback, per the CEI reorder) must defend against.
...
/// @notice ENG-3145: burning a token to zero supply must not let the original minter re-mint
/// the same id and overwrite its property data, on EITHER the `mint` or `mintBatch` path (they
/// derive the same id from generateId and write the same `_property` slot). Also covers N-1:
/// the `_mint` CEI reorder (guard flag + property written before the receiver callbacks).
contract FabricaTokenBurnRemintTest is Test {
    ...
    // Storage slot of the _everMinted mapping (appended after _contractURI at 306).
    ...
    function test_mint_burnToZero_remintSameId_reverts() public {
        ...
        // Burn the full supply. `_burn` zeroes supply but never clears `_everMinted`, so the id
    ...
    /// `mintBatch` (same generateId → same `_property` slot → same `_everMinted` flag).
    ...
    /// the case the validator-sentinel approach could not handle.
    ...
    /// @dev N-1: a re-entrant `mint` during `onERC1155Received` cannot bypass the guard or
    /// corrupt `_property`. The `_everMinted` flag + property are written BEFORE the callback,
    /// so the re-entrant same-id mint reverts; the outer mint completes with its own data and an
    /// un-inflated balance.
    ...
    /// @dev UPGRADE SAFETY (regression): a token minted BEFORE this upgrade has
    /// `_everMinted[id] == false` (its slot was never written). The guard must still block
    /// re-minting it while it is LIVE — the `supply == 0` term carries that protection. Model
    /// the post-upgrade state of a live legacy token by clearing `_everMinted[id]` while leaving
    /// supply > 0, then assert a same-id re-mint reverts and the property is untouched. Without
    /// the `supply == 0` term this test fails (the live token would be overwritable).
    function test_mint_preUpgradeLiveToken_cannotBeReminted() public {
        vm.prank(operator);
        uint256 id = _mint(8, 100, "ParcelA", "ipfs://oa-8");
        // Simulate "minted before this upgrade": _everMinted unset, token still live.
        bytes32 slot = keccak256(abi.encode(id, SLOT_EVER_MINTED));
        vm.store(address(token), slot, bytes32(0));
        assertEq(uint256(vm.load(address(token), slot)), 0, "precondition: _everMinted cleared");
        _assertProperty(id, 100, "ipfs://oa-8", "ParcelA", address(validator));
        ...
        vm.prank(operator);
        vm.expectRevert(bytes(REVERT_MSG));
        token.mint(recipients, 8, amounts, "ParcelB", "ipfs://oa-8", "{}", address(validator));

        _assertProperty(id, 100, "ipfs://oa-8", "ParcelA", address(validator));
        assertEq(token.balanceOf(operator, id), 100, "balance must not be inflated");
    }

    /// @dev UPGRADE SAFETY on the batch path: a live pre-upgrade token (flag cleared, supply>0)
    /// cannot be re-minted via `mintBatch` either — the `supply == 0` term carries the protection.
    function test_mintBatch_preUpgradeLiveToken_cannotBeReminted() public {
        vm.prank(operator);
        uint256 id = _mintBatchSingle(8, 100, "ParcelA", "ipfs://oa-8");
        bytes32 slot = keccak256(abi.encode(id, SLOT_EVER_MINTED));
        vm.store(address(token), slot, bytes32(0));
        assertEq(uint256(vm.load(address(token), slot)), 0, "precondition: _everMinted cleared");
