...
contract FabricaFeeCollectorScript is Script {
    ...
    function run(
        ...
        bytes memory initializeData = abi.encodeWithSignature(
            "initialize(address,uint8,address)", protocolContract, protocolSharePercent, protocolFeeRecipient
        );
        FabricaProxy proxy = new FabricaProxy(address(implementation), proxyAdmin, initializeData);
