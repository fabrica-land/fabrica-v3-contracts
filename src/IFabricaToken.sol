...
interface IFabricaToken {
    ...
    function _property(uint256 tokenId)
        external
        view
        returns (
            uint256 supply,
    ...
    function burn(address from, uint256 id, uint256 amount) external returns (bool success);

    function burnBatch(address from, uint256[] memory ids, uint256[] memory amounts) external returns (bool success);
    ...
    function updateOperatingAgreement(string memory operatingAgreement, uint256 id) external returns (bool);

    // @dev only executable by > 50% owner
    function updateConfiguration(string memory configuration, uint256 id) external returns (bool);

    // @dev only executable by > 70% owner
    function updateValidator(address validator, uint256 id) external returns (bool);
}
