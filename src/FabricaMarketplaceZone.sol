...
contract FabricaMarketplaceZone is ZoneInterface {
    ...
    function authorizeOrder(ZoneParameters calldata p) external view returns (bytes4) {
        _verify(p);
        return _AUTHORIZE_MAGIC;
    }

    function validateOrder(ZoneParameters calldata p) external view returns (bytes4) {
    ...
    function _verify(ZoneParameters calldata p) internal view {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                _DOMAIN_SEPARATOR,
                keccak256(abi.encode(_EIP712_TYPE_HASH, p.orderHash, expiry, keccak256(defUrl), keccak256(dpId)))
            )
        );
    ...
    function _verifyDefinitionUrl(ZoneParameters calldata p, bytes memory signedDefUrl) internal view {
        // ItemType.ERC1155 = 3
    ...
    function _checkDefinition(address tokenContract, uint256 tokenId, bytes memory signedDefUrl) internal view {
